        ...
        gpointer user_data)
{
    ...
    gum_dbus_user_call_update_mine_finish (proxy, res, &error);
        ...
        gpointer user_data)
{
    ...
    if (uid != GUM_USER_INVALID_UID && uid == getuid()) {
        gum_dbus_user_call_update_mine (self->priv->dbus_user,
                self->priv->cancellable, _on_user_update_mine_cb, self);
    } else {
        ...
        GumUser *self)
{
    ...
    if (self->priv->offline_user) {
        ...
        g_object_get (G_OBJECT (self), "uid", &uid, NULL);
        if (uid != GUM_USER_INVALID_UID && uid == getuid()) {
            rval = gum_dbus_user_call_update_mine_sync (self->priv->dbus_user,
